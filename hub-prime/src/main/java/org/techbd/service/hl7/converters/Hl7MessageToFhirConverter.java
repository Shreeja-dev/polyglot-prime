...
import org.techbd.service.hl7.core.TemplateFactory;
...
public class Hl7MessageToFhirConverter {
    ...
    public Bundle convert(Message message, String interactionId) throws Exception {
        ...
        if (null != messageType) {
            ...
            if (templateOpt.isPresent()) {
                ...
                IMessage messageObj = template.copyFromHAPI(message);
                bundle = processSegments(template,messageObj,interactionId);               
            } else {
                LOG.error(
    ...
    public Bundle processSegments(ITemplate template, IMessage messageObj, String interactionId) {
        List<String> supportedSegments = template.getSegments();
        Bundle bundle = generateEmptyBundle();
        supportedSegments.forEach(segment -> {
            Hl7SegmentType segmentType = switch (segment) {
                case String s when Hl7SegmentType.fromString(s) != null -> Hl7SegmentType.fromString(s);
                default -> throw new IllegalArgumentException("Segment not specified in Hl7SegmentType: " + segment);
            };
            IHl7SegmentToFhirConverter converter = findConverter(segmentType);
            if (converter != null) {
                try {
                    BundleEntryComponent bundleEntry = converter.convert(template, interactionId);
