...
public class CsvService {
     ...
     *                   parsing.
     */
    public List<Object> processZipFile(final MultipartFile file,final HttpServletRequest request ,HttpServletResponse response ,final String tenantId,String origin,String sftpSessionId,String provenance) throws Exception {
        CsvOrchestrationEngine.OrchestrationSession session = null;
        try {
            final var dslContext = udiPrimeJpaConfig.dsl();
            final var jooqCfg = dslContext.configuration();
            ...
            session.getPayloadAndValidationOutcomes(), session.getFilesNotProcessed(),request,
             response,tenantId,file.getOriginalFilename(),provenance);
        } finally {
            if (null == session) {
                engine.clear(session);
            }
