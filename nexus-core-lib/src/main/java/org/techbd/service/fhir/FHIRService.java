...
//     }
 ...
 /**
        ...
        try {
            ...
            final var jooqCfg = dslContext.configuration();
			 registerOriginalPayload(jooqCfg, headerParameters, requestParameters,
						...
						registerOriginalPayload(org.jooq.Configuration jooqCfg,
			Map<String, String> headerParameters,
            ...
            try {
                ...
                if (null == payloadWithDisposition) {
                    LOG.warn(
                            ...
                            interactionId);
                    sendToScoringEngine(jooqCfg, headerParameters, requestParameters,customDataLakeApi, dataLakeApiContentType,
                            tenantId, payload,
                            provenance, null,
                            mtlsStrategy,
                ...
                payloadWithDisposition = registerBundleInteraction(jooqCfg, headerParameters, requestParameters,
                        responseParameters, payload, buildOperationOutcome(ex, interactionId), interactionId,
	...
	private Map<String, Object> registerOriginalPayload(org.jooq.Configuration jooqCfg,
			Map<String, String> headerParameters,
			Map<String, String> requestParameters,
			String payload,
			String interactionId,
		...
		Span span = tracer.spanBuilder("FHIRService.registerOriginalPayload").startSpan();
		try {
	...
	private Map<String, Object> registerValidationResults(org.jooq.Configuration jooqCfg,
			Map<String, String> headerParameters,
			Map<String, String> requestParameters,
			...
			String masterInteractionId,
			String sourceType,
			String requestUriToBeOverriden) throws IOException {
		Span span = tracer.spanBuilder("FHIRService.registerValidationResults").startSpan();
		try {
			final var rihr = new RegisterInteractionFhirRequest();
			final var provenance = "%s.doFilterInternal".formatted(FHIRService.class.getName());
			final var start = Instant.now();
			...
			prepareRequestBase(
					...
					provenance,
					"TechByDisposistion",
					Configuration.objectMapper.valueToTree(immediateResult));
	...
	private void prepareRequestBase(RegisterInteractionFhirRequest rihr,
			String interactionId,
			String groupInteractionId,
			String masterInteractionId,
			String sourceType,
	...
	private void sendToScoringEngine(org.jooq.Configuration jooqCfg, Map<String,String> headerParameters,
			Map<String,String> requestParameters,
			...
			try {
				...
				if ( null != validationPayloadWithDisposition) { // todo
					...
					LOG.debug(
							...
							interactionId);
					bundlePayloadWithDisposition = preparePayload(headerParameters,
							payload,
				...
				if (null == defaultDatalakeApiAuthn) {
					...
					handleNoMtls(MTlsStrategy.NO_MTLS, interactionId, tenantId, dataLakeApiBaseURL,
							jooqCfg, requestParameters,headerParameters,
					...
					handleMTlsStrategy(defaultDatalakeApiAuthn, interactionId, tenantId,
							dataLakeApiBaseURL,
							jooqCfg, requestParameters,headerParameters, bundlePayloadWithDisposition,
							payload,
	...
	public void handleMTlsStrategy(DefaultDataLakeApiAuthn defaultDatalakeApiAuthn, String interactionId,
			...
			org.jooq.Configuration jooqCfg, Map<String,String> requestParameters, Map<String,String> headerParameters,
		...
		switch (mTlsStrategy) {
			...
			case MTLS_RESOURCES ->
				handleMtlsResources(interactionId, tenantId, jooqCfg,
						bundlePayloadWithDisposition,payload, provenance, requestParameters,headerParameters,
			...
			case WITH_API_KEY ->
				...
				jooqCfg, requestParameters, headerParameters,bundlePayloadWithDisposition, payload, dataLakeApiContentType,
	...
	private void handleMtlsResources(String interactionId, String tenantId, org.jooq.Configuration jooqCfg,
			...
			String payload, String provenance,Map<String,String> requestParameters,Map<String,String> headerParameters,String dataLakeApiContentType,
		...
		try {
			...
			sendPostRequest(webClient, tenantId, bundlePayloadWithDisposition, payload,
					...
					jooqCfg, provenance, requestParameters.get(Constants.REQUEST_URI), dataLakeApiBaseURL,
					groupInteractionId, masterInteractionId, sourceType,bundleId);
			LOG.debug(
	...
	private void handleApiKeyAuth(String interactionId, String tenantId,
		...
		LOG.debug("FHIRService:: handleNoMtls Build WebClient with MTLS  Disabled -BEGIN \n"
				...
				interactionId,
				tenantId);
		var webClient = createWebClient(dataLakeApiBaseURL, jooqCfg, requestParameters,headerParameters,
				tenantId, payload,
	...
	private void handlePostStdoutPayload(String interactionId, String tenantId, org.jooq.Configuration jooqCfg,
			...
			String requestUriToBeOverriden,Map<String,String> requestParameters) {
	...
	private WebClient createWebClient(String scoringEngineApiURL,
			...
			Map<String,String> requestParamters,
		...
		return WebClient.builder()
				...
				.filter(ExchangeFilterFunction.ofRequestProcessor(clientRequest -> {
					filter(clientRequest, requestParamters,headerParamters, jooqCfg, provenance, tenantId, payload,
							bundlePayloadWithDisposition,
	...
	private Map<String, Object> preparePayload(Map<String,String> headerParameters, String bundlePayload,
		...
		try {
			Map<String, Object> extractedOutcome = Optional
					.ofNullable(extractIssueAndDisposition(interactionId, payloadWithDisposition, headerParameters))
    ...
    public Map<String, Object> extractIssueAndDisposition(String interactionId,
            Map<String, Object> operationOutcomePayload, Map<String,String>headerParameters) {
        ...
        return Optional.ofNullable(operationOutcomePayload.get("OperationOutcome"))
                ...
                .flatMap(operationOutcomeMap -> {
                    ...
                    String headerSeverityLevelValue = headerParameters.get(Constants.VALIDATION_SEVERITY_LEVEL);
	...
	private void registerStateForward(org.jooq.Configuration jooqCfg, String provenance,
		...
		try {
			...
			try {
				...
				initRIHR.setPFromState("DISPOSITION");
				initRIHR.setPToState("FORWARD");
	...
	private void registerStateComplete(org.jooq.Configuration jooqCfg, String bundleAsyncInteractionId,
		...
		try {
			...
			try {
				...
				forwardRIHR.setPFromState("FORWARD");
				forwardRIHR.setPToState("COMPLETE");
				...
				final var end = Instant.now();
				JsonNode responseFromDB = forwardRIHR.getReturnValue();
				Map<String,Object> responseAttributes = CoreFHIRUtil.extractFields(responseFromDB);
				LOG.info(
	...
	private void registerStateFailure(org.jooq.Configuration jooqCfg, String dataLakeApiBaseURL,
		...
		try {
			...
			try {
				...
				errorRIHR.setPFromState("FORWARD");
				errorRIHR.setPToState("FAIL");
				...
				JsonNode responseFromDB = errorRIHR.getReturnValue();
				Map<String, Object> responseAttributes = CoreFHIRUtil.extractFields(responseFromDB);
